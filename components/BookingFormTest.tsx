...
import { STRIPE_TOUR_PRODUCTSTest, STRIPE_ADD_ONS, getProductByGroupSize, calculateTotalPrice, redirectToStripeCheckout, type StripeProduct, type StripeAddOn } from "@/lib/stripe-products";
...
const TOUR_OPTIONS = STRIPE_TOUR_PRODUCTSTest;
...
export default function BookingForm({ onClose, isModal = false }: BookingFormProps) {
  ...
  const handleInputChange = (field: keyof BookingData, value: string | boolean | string[] | number) => {
    ...
    if (field === 'tourOption') {
      const selectedProduct = STRIPE_TOUR_PRODUCTSTest.find(p => p.id === value as string) || null;
    ...
    if (field === 'numberOfTravelers' || field === 'tourOption' || field === 'adultsCount' || field === 'childrenCount' || field === 'childrenUnder5Count') {
      ...
      if (field === 'tourOption') {
        selectedProduct = STRIPE_TOUR_PRODUCTSTest.find(p => p.id === value as string) || null;
      } else {
